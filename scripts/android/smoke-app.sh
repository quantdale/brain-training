...
# 1. App installed?
if ! bt_adb -s "$SERIAL" shell pm list packages | grep -q "^package:${APP_ID}$"; then
...
# 2. Launch
if ! bt_adb -s "$SERIAL" shell am force-stop "$APP_ID"; then
  step "launch" "FAIL" 1
  bt_log "ERROR: am force-stop failed"
  exit 1
fi
if ! bt_adb -s "$SERIAL" shell am start -W -n "${APP_ID}/${ACTIVITY}" >/dev/null 2>&1; then
  step "launch" "FAIL" 1
...
for i in $(seq 1 "$WAIT_SECONDS"); do
  sleep 1
  FOREGROUND="$(bt_adb -s "$SERIAL" shell dumpsys window 2>/dev/null | grep -o "mCurrentFocus=.*" | head -1)"
  if echo "$FOREGROUND" | grep -q "${APP_ID}"; then
    break
  fi
done
if ! echo "$FOREGROUND" | grep -q "${APP_ID}"; then
  bt_adb -s "$SERIAL" exec-out screencap -p > "$RUN_DIR/screenshots/launch-fail-01.png" 2>/dev/null || true
  bt_adb -s "$SERIAL" logcat -d > "$RUN_DIR/logcat/logcat.txt" 2>/dev/null || true
...
sleep 8
if bt_adb -s "$SERIAL" exec-out screencap -p > "$RUN_DIR/screenshots/home-01.png" 2>/dev/null &&
   [ -s "$RUN_DIR/screenshots/home-01.png" ] &&
   [ "$(head -c 8 "$RUN_DIR/screenshots/home-01.png" | od -An -tx1 | tr -d ' \n')" = "89504e470d0a1a0a" ]; then
  step "screenshot-home" "PASS" 0
else
  step "screenshot-home" "FAIL" 1
fi

if bt_shell "$SERIAL" 120 uiautomator dump /sdcard/smoke-home.xml >/dev/null 2>&1 &&
   bt_pull "$SERIAL" /sdcard/smoke-home.xml "$RUN_DIR/hierarchy/home-hierarchy.xml" >/dev/null 2>&1 &&
   [ -s "$RUN_DIR/hierarchy/home-hierarchy.xml" ]; then
  step "hierarchy-home" "PASS" 0
else
  step "hierarchy-home" "FAIL" 1
fi

if bt_adb -s "$SERIAL" logcat -d > "$RUN_DIR/logcat/logcat.txt" 2>/dev/null &&
   [ -s "$RUN_DIR/logcat/logcat.txt" ]; then
  if ! bt_adb -s "$SERIAL" logcat -d -v brief "*:W" > "$RUN_DIR/logcat/logcat-filtered.txt" 2>/dev/null; then
    bt_warn "filtered logcat capture failed; full logcat is still available"
  fi
  step "logcat" "PASS" 0
else
  step "logcat" "FAIL" 1
...
bt_log "smoke failed: $FAILURES step(s) failed"
exit 1
