 ...
 * - Simple average of all domain ratings
 * - Unseen domains (never played) are treated as at INITIAL_RATING (1000)
 * - Stale domains (not played recently) are weighted less
 *
...
export function computeComposite(
  ...
  for (const domain of knownDomains) {
    ...
    if (rating) {
      ...
      const isStale = (nowMs - rating.updatedAt) > staleThresholdMs;
      
      // Weight stale domains less (0.5 weight)
      const weight = isStale ? 0.5 : 1;
      ...
      // Domain never played - treat as INITIAL_RATING with full weight
