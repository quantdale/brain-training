...
describe('createMathQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createMathQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createMathQaForceStateHooks(jest.fn());
...
describe('createMathTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createMathTutorialLifecycle();
