/**
 * Session result building + persistence for the Spatial Fold Match game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: SpatialFoldMatchDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: SpatialFoldMatchStats;
 ...
 */
export function buildSpatialFoldMatchRawResult(
  input: BuildRawResultInput,
): SpatialFoldMatchRawResult {
  const generatorInfo = {
    gridRows: input.params.gridRows,
    gridCols: input.params.gridCols,
    filledCells: input.params.filledCells,
    optionCount: input.params.optionCount,
    rounds: input.params.rounds,
    sourceRevealMs: input.params.sourceRevealMs,
  ...
  const averageAnswerMs =
    input.stats.roundsPlayed > 0
      ? Math.round(input.stats.totalAnswerMs / input.stats.roundsPlayed)
      : 0;
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: SpatialFoldMatchRawResult;
 ...
 */
export async function persistSpatialFoldMatchSession(
