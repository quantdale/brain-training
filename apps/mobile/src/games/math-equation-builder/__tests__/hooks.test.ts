...
describe('createMathEquationBuilderQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createMathEquationBuilderQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createMathEquationBuilderQaForceStateHooks(jest.fn());
...
describe('createMathEquationBuilderTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createMathEquationBuilderTutorialLifecycle();
