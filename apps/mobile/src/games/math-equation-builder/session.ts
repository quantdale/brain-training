/**
 * Session result building + persistence for the Equation Builder game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly stats: MathEquationBuilderStats;
 ...
 */
export function buildMathEquationBuilderRawResult(
  input: BuildRawResultInput,
): MathEquationBuilderRawResult {
  const generatorInfo = {
    numbersCount: input.params.numbersCount,
    targetMin: input.params.targetMin,
    targetMax: input.params.targetMax,
  ...
  return {
    ...
    totalTimeBonus: input.stats.totalTimeBonus,
    puzzlesSolvedFirstTry: input.stats.puzzlesSolvedFirstTry,
    numbersCount: input.params.numbersCount,
    targetMin: input.params.targetMin,
    targetMax: input.params.targetMax,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: MathEquationBuilderRawResult;
 ...
 */
export async function persistMathEquationBuilderSession(
  ...
  try {
    ...
    console.error(
      `[math-equation-builder] failed to persist completed session ${record.id}`,
      error,
    );
