...
import {
  createOddOneOutQaForceStateHooks,
...
describe('createOddOneOutQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createOddOneOutQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createOddOneOutQaForceStateHooks(jest.fn());
...
describe('createOddOneOutTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createOddOneOutTutorialLifecycle();
