/**
 * Session result building + persistence for the Odd One Out game.
 *
...
import { accuracyOf, firstTryRateOf, avgSolveRatioOf } from './scoring';
import { GAME_ID } from './types';
import type { OddOneOutDifficultyParams, OddOneOutRawResult, OddOneOutStats } from './types';
...
export interface BuildRawResultInput {
  ...
  readonly params: OddOneOutDifficultyParams;
  /** Effective round params of the final round (full progression is replayable). */
  readonly gridSize: number;
  readonly subtlety: number;
  readonly windowMs: number;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: OddOneOutStats;
 ...
 */
export function buildOddOneOutRawResult(input: BuildRawResultInput): OddOneOutRawResult {
  const generatorInfo = {
    gridSize: input.gridSize,
    subtlety: input.subtlety,
    windowMs: input.windowMs,
    rounds: input.params.rounds,
    minSubtlety: input.params.minSubtlety,
    maxSubtlety: input.params.maxSubtlety,
    minWindowMs: input.params.minWindowMs,
    maxWindowMs: input.params.maxWindowMs,
  ...
  return {
    ...
    firstTryCorrect: input.stats.firstTryCorrect,
    ...
    firstTryRate: firstTryRateOf(input.stats.firstTryCorrect, input.stats.roundsPlayed),
    avgSolveRatio: avgSolveRatioOf(input.stats.solveRatioSum, input.stats.roundsPassed),
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: OddOneOutRawResult;
 ...
 */
export async function persistOddOneOutSession(
  ...
  try {
    ...
    console.error(`[attention-odd-one-out] failed to persist completed session ${record.id}`, error);
