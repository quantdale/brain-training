...
import type { MemoryRawResult, MemoryStats } from '../types';
...
function buildRaw(overrides: Partial<Parameters<typeof buildMemoryRawResult>[0]> = {}): MemoryRawResult {
  return buildMemoryRawResult({
    gameVersion: '1.0.0',
    generatorVersion: '1.0.0',
    scoringVersion: '1.0.0',
    difficulty: 'normal',
    params: MEMORY_DIFFICULTY_PARAMS.normal,
    challengeRating: 0.5,
    seed: '42',
    stats: STATS,
    forced: false,
...
describe('buildSessionRecord', () => {
  it('maps the session onto the db record shape', () => {
    ...
    const record: GameSessionRecord = buildSessionRecord({
      sessionId: 'memory-x1',
      rawResult: raw,
      difficulty: { level: 'normal', challengeRating: 0.5, parameters: { ...MEMORY_DIFFICULTY_PARAMS.normal } },
      normalized,
      xp: 0,
      startedAtMs: 1_000,
...
describe('persistMemorySession', () => {
  ...
  it('persists through the injected persister and reports the result', async () => {
    ...
    const outcome = await persistMemorySession(record, persister);
  ...
  it('logs and returns failure instead of throwing when the db fails', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      '[memory] failed to persist completed session memory-p1',
      expect.any(Error),
    );
  });
});
