...
function fullSession(seed: string, gridSize = 9, startLength = 4, rounds = 5): number[][] {
  ...
  for (let round = 0; round < rounds; round += 1) {
    const sequence = generateRoundSequence({ rng, roundIndex: round, length, gridSize, prevSequence: prev });
...
describe('generateRoundSequence', () => {
  ...
  it('draws a permutation slice: correct length, distinct tiles, in range', () => {
    const rng = createRng('perm-check');
    const sequence = generateRoundSequence({ rng, roundIndex: 0, length: 5, gridSize: 9, prevSequence: null });
  ...
  it('works on the 16-tile grid too', () => {
    ...
    const sequence = generateRoundSequence({ rng, roundIndex: 0, length: 6, gridSize: 16, prevSequence: null });
  ...
  it('is bounded: generation always terminates deterministically', () => {
    ...
    const sequence = generateRoundSequence({ rng, roundIndex: 1, length: 4, gridSize: 9, prevSequence: previous });
