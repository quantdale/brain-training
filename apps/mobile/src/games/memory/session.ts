/**
 * Session result building + persistence for the Memory game.
 *
...
import type { MemoryDifficultyParams, MemoryRawResult, MemoryStats } from './types';
...
export interface BuildRawResultInput {
  ...
  readonly params: MemoryDifficultyParams;
 ...
 */
export function buildMemoryRawResult(input: BuildRawResultInput): MemoryRawResult {
  const generatorInfo = {
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: MemoryRawResult;
 ...
 */
export async function persistMemorySession(
  ...
  try {
    ...
    console.error(`[memory] failed to persist completed session ${record.id}`, error);
