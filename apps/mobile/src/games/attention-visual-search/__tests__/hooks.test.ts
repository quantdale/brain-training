...
import {
  createVisualSearchQaForceStateHooks,
  createVisualSearchTutorialLifecycle,
} from '../hooks';
import { GAME_ID } from '../types';

describe('createVisualSearchQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn();
    const hooks = createVisualSearchQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createVisualSearchQaForceStateHooks(jest.fn());
...
describe('createVisualSearchTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createVisualSearchTutorialLifecycle();
