/**
 * Session result building + persistence for the Visual Search game.
 *
 ...
 */
import { RNG_ALGORITHM_VERSION, createDiagnosticMetadata } from '@/sdk';
import type {
  DiagnosticMetadata,
  DifficultyLevel,
  DifficultyProfile,
...
import { DISTRACTOR_PENALTY_MS } from './difficulty';
import { accuracyOf, avgResponseMsOf, avgSpeedRatio } from './scoring';
...
export interface BuildRawResultInput {
  ...
  readonly params: VisualSearchDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: VisualSearchStats;
 ...
 */
export function buildVisualSearchRawResult(input: BuildRawResultInput): VisualSearchRawResult {
  const generatorInfo = {
    initialGridSize: input.params.initialGridSize,
    maxGridSize: input.params.maxGridSize,
    initialWindowMs: input.params.initialWindowMs,
    minWindowMs: input.params.minWindowMs,
    windowStepMs: input.params.windowStepMs,
    sessionDurationMs: input.params.sessionDurationMs,
    rounds: input.params.rounds,
    distractorPenaltyMs: DISTRACTOR_PENALTY_MS,
  ...
  return {
    ...
    avgResponseMs: avgResponseMsOf(input.stats.sumResponseMs, input.stats.roundsPassed),
    fastestResponseMs: input.stats.fastestResponseMs,
    avgSpeedRatio: avgSpeedRatio(input.stats.sumResponseRatio, input.stats.roundsPassed),
    initialGridSize: input.params.initialGridSize,
    maxGridSize: input.params.maxGridSize,
    initialWindowMs: input.params.initialWindowMs,
    minWindowMs: input.params.minWindowMs,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: VisualSearchRawResult;
 ...
 */
export async function persistVisualSearchSession(
  ...
  try {
    ...
    console.error(
      `[attention-visual-search] failed to persist completed session ${record.id}`,
      error,
    );
