/**
 * Value Order game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createValueOrderingQaForceStateHooks` implements the SDK
...
import type { ValueOrderingAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createValueOrderingQaForceStateHooks(
  dispatch: Dispatch<ValueOrderingAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
 ...
 */
export function createValueOrderingTutorialLifecycle(
