...
import {
  createLanguageQaForceStateHooks,
...
describe('createLanguageQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createLanguageQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createLanguageQaForceStateHooks(jest.fn());
...
describe('createLanguageTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createLanguageTutorialLifecycle();
