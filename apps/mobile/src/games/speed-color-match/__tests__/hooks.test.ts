...
describe('createSpeedColorMatchQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createSpeedColorMatchQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createSpeedColorMatchQaForceStateHooks(jest.fn());
...
describe('createSpeedColorMatchTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createSpeedColorMatchTutorialLifecycle();
