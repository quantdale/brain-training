/**
 * Speed Color Match game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createSpeedColorMatchQaForceStateHooks` implements the SDK
...
export function createSpeedColorMatchQaForceStateHooks(
  dispatch: Dispatch<SpeedColorMatchAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Speed Color Match game. Uses the SDK reference
 * lifecycle over the provided store (an in-memory store by default).
 */
export function createSpeedColorMatchTutorialLifecycle(
