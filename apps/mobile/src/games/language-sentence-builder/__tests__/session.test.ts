...
import type { SentenceBuilderRawResult, SentenceBuilderStats } from '../types';

const STATS: SentenceBuilderStats = {
  score: 550,
  roundsPlayed: 4,
  roundsPassed: 3,
  bestStreak: 2,
...
function buildRaw(overrides: Partial<Parameters<typeof buildRawResult>[0]> = {}): SentenceBuilderRawResult {
...
describe('buildRawResult', () => {
  it('carries the full reproducibility envelope', () => {
    ...
    expect(raw.avgWordLengthFactor).toBe(0.6);
  ...
  it('reflects the stats', () => {
    ...
    expect(raw.longestSentence).toBe(7);
...
describe('buildSessionRecord', () => {
  it('maps the session onto the db record shape', () => {
    const raw = buildRaw();
    const normalized = normalizeSentenceBuilderResult(raw, {
      gameId: 'language-sentence-builder',
      difficulty: 'normal',
      durationMs: 45_000,
    });
    const record: GameSessionRecord = buildSessionRecord({
      ...
      parameters: {
        ...
        rounds: DIFFICULTY_PARAMS.normal.rounds,
        timeBudgetMs: DIFFICULTY_PARAMS.normal.timeBudgetMs,
      },
    },
      normalized,
      xp: 0,
...
describe('persistSession', () => {
  ...
  it('logs and returns failure instead of throwing when the db fails', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      '[sentence-builder] failed to persist completed session sb-p1',
      expect.any(Error),
    );
  });
});
