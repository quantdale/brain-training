/**
 * Sentence Builder game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createSentenceBuilderQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { SentenceBuilderAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createSentenceBuilderQaForceStateHooks(
  dispatch: Dispatch<SentenceBuilderAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/** Tutorial lifecycle for the Sentence Builder game. */
export function createSentenceBuilderTutorialLifecycle(
