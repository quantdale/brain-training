/**
 * Session result building + persistence for the Spatial Grid Navigator game.
 *
...
import { accuracyOf, hardAccuracyOf, speedScoreOf } from './scoring';
...
export interface BuildRawResultInput {
  ...
  readonly params: SpatialGridNavDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: SpatialGridNavStats;
 ...
 */
export function buildRawResult(input: BuildRawResultInput): SpatialGridNavRawResult {
  const params = input.params;
  const averageResponseMs =
    input.stats.scoredPicks > 0
      ? Math.round(input.stats.totalResponseMs / input.stats.scoredPicks)
      : 0;
  const speedScore = speedScoreOf(averageResponseMs, params.speedTargetMs);
  const accuracy = accuracyOf(input.stats.correctPicks, input.stats.roundsPlayed);
  const hardAccuracy = hardAccuracyOf(input.stats.hardCorrect, input.stats.hardPlayed);
  ...
  return {
    ...
    averageResponseMs,
    speedScore,
    hardPlayed: input.stats.hardPlayed,
    hardCorrect: input.stats.hardCorrect,
    hardAccuracy,
    gridSide: params.gridSide,
    minCommandCount: params.minCommandCount,
    maxCommandCount: params.maxCommandCount,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: SpatialGridNavRawResult;
 ...
 */
export async function persistSpatialGridNavSession(
  ...
  try {
    ...
    console.error(`[${GAME_ID}] failed to persist completed session ${record.id}`, error);
