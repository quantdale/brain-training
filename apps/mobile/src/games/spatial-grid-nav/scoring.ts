/**
 * Scoring + normalization for the Spatial Grid Navigator game.
 *
 * Raw scoring is game-owned; `normalizeSpatialGridNavResult` converts it to the
 ...
 *   hardAccuracy  = hardPlayed>0 ? hardCorrect/hardPlayed : 0
 *   value         = clamp01(accuracy * (0.6 + 0.2*speed + 0.2*hardAccuracy))
 *
 * Accuracy is the base; speed contributes up to 20% and hard-round accuracy
 * up to 20%. Difficulty itself is deliberately NOT folded into the value — it
 * is recorded on the raw result / diagnostic metadata so the Phase-2 rating
 * pipeline can weight it.
 */
import type { NormalizeContext, NormalizedPerformance, PerformanceNormalizer } from '@/sdk';

import { GAME_ID } from './types';
import type { SpatialGridNavDifficultyParams, SpatialGridNavRawResult } from './types';
...
export function perfectSessionScore(params: SpatialGridNavDifficultyParams): number {
...
/** Hard-round accuracy; 0 when no hard rounds were played. */
export function hardAccuracyOf(hardCorrect: number, hardPlayed: number): number {
...
export function normalizeSpatialGridNavResult(
  raw: SpatialGridNavRawResult,
  ...
  const hardAccuracy = hardAccuracyOf(raw.hardCorrect, raw.hardPlayed);
  const value = clamp01(accuracy * (0.6 + 0.2 * speed + 0.2 * hardAccuracy));
  return { value, scale: '0..1', raw: { ...raw } };
}

/** SDK-conformant normalizer instance for the Spatial Grid Navigator game. */
export const spatialGridNavPerformanceNormalizer: PerformanceNormalizer<SpatialGridNavRawResult> = {
  gameId: GAME_ID,
  normalize: normalizeSpatialGridNavResult,
};
