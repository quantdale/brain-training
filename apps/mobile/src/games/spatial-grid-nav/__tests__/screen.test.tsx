...
describe('SpatialGridNavScreen', () => {
  ...
  it('renders the intro with difficulty options and starts a session', async () => {
    ...
    expect(screen.getByTestId(testId(GAME_ID, 'options-grid'))).toBeOnTheScreen();
  ...
  it('opens the tutorial on first play and can be skipped (dev)', async () => {
    ...
    expect(screen.getByTestId(testId(GAME_ID, 'tutorial-board'))).toBeOnTheScreen();
    expect(screen.getByTestId(testId(GAME_ID, 'tutorial-commands'))).toBeOnTheScreen();
    ...
    await render(<SpatialGridNavScreen clock={createFakeClock()} tutorialStore={store} sessionSeed="tut" />);
  ...
  it('plays a full normal session end-to-end and persists the record', async () => {
    ...
    for (let round = 0; round < rounds; round += 1) {
      ...
      await fireEvent.press(screen.getByTestId(testId(GAME_ID, 'option', String(correctIndex))));
      if (round < rounds - 1) {
  ...
  it('pauses: the overlay appears, blocks picks, and resumes', async () => {
    ...
    // Picks are ignored while paused (options are hidden behind the overlay).
    await fireEvent.press(screen.getByTestId(testId(GAME_ID, 'option', '0'), { hidden: true }));
  ...
  it('force-win ends the session as a perfect run and marks it forced', async () => {
    ...
    expect(persister.completeSession).toHaveBeenCalledTimes(1);
    const input = persister.completeSession.mock.calls[0][0] as CompleteSessionInput;
    expect((input.session.rawResult as SpatialGridNavRawResult).forced).toBe(true);
