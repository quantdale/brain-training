...
describe("createWordChainQaForceStateHooks", () => {
  it("dispatches the matching reducer action for each hook", () => {
    const dispatch = jest.fn();
    const hooks = createWordChainQaForceStateHooks(dispatch);
  ...
  it("is bound to the game id and runs in dev builds", () => {
    const hooks = createWordChainQaForceStateHooks(jest.fn());
...
describe("createWordChainTutorialLifecycle", () => {
  it("shows on first play, completes, and honors replays", () => {
    const tutorial = createWordChainTutorialLifecycle();
