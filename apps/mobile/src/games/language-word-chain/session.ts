/**
 * Session result building + persistence for the Word Chain game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly stats: LanguageWordChainStats;
 ...
 * full reproducibility envelope (seed, versions, difficulty, per-chain
 ...
 */
export function buildWordChainRawResult(
  ...
  const generatorInfo = {
    ...
    minChainLen: input.params.minChainLen,
    maxChainLen: input.params.maxChainLen,
    minBlanks: input.params.minBlanks,
    maxBlanks: input.params.maxBlanks,
    optionsPerStep: input.params.optionsPerStep,
  ...
  return {
    ...
    stepsPlayed: input.stats.stepsPlayed,
    stepsCorrect: input.stats.stepsCorrect,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: LanguageWordChainRawResult;
 ...
 */
export async function persistWordChainSession(
  ...
  try {
    ...
    console.error(
      `[language-word-chain] failed to persist completed session ${record.id}`,
      error,
    );
