...
describe('createSpatialQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createSpatialQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createSpatialQaForceStateHooks(jest.fn());
...
describe('createSpatialTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createSpatialTutorialLifecycle();
