/**
 * Session result building + persistence for the Mental Rotation game.
 *
...
import { accuracyOf, speedOf } from './scoring';
import { GAME_ID } from './types';
import type { SpatialDifficultyParams, SpatialRawResult, SpatialStats } from './types';
 ...
 */
export function buildSpatialRawResult(input: BuildRawResultInput): SpatialRawResult {
  const generatorInfo = {
    blocks: input.params.blocks,
    angleMask: input.params.angleMask,
  ...
  return {
    ...
    speed: speedOf(input.stats.totalRemainingMs, input.stats.totalBudgetMs),
    bestStreak: input.stats.bestStreak,
    totalAnswers: input.stats.totalAnswers,
    correctAnswers: input.stats.correctAnswers,
    timeouts: input.stats.timeouts,
    totalRemainingMs: input.stats.totalRemainingMs,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: SpatialRawResult;
 ...
 */
export async function persistSpatialSession(
  ...
  try {
    ...
    console.error(`[spatial-mental-rotation] failed to persist completed session ${record.id}`, error);
