/**
 * Session result building + persistence for the Tap Rush game.
 *
...
import { accuracyOf, bestOf, meanOf, meanSpeedOf } from './scoring';
import { GAME_ID } from './types';
import type { TapRushDifficultyParams, TapRushRawResult, TapRushStats } from './types';
...
export interface BuildRawResultInput {
  ...
  readonly params: TapRushDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: TapRushStats;
  /** Response window of the final round (drives the adaptive rating). */
...
/**
 * Build the persisted raw result. It carries the tap statistics plus the full
 * reproducibility envelope (seed, versions, difficulty, generator info) per
 * the SDK generator rule and constitution §21.
 */
export function buildTapRushRawResult(input: BuildRawResultInput): TapRushRawResult {
  const generatorInfo = {
    count: input.params.count,
    rounds: input.params.rounds,
    initialWindowMs: input.params.initialWindowMs,
    minWindowMs: input.params.minWindowMs,
    windowStepMs: input.params.windowStepMs,
    targetRadius: input.params.targetRadius,
  ...
  return {
    ...
    totalTargets: input.params.count * input.params.rounds,
    targetsHit: input.stats.targetsHit,
    targetsMissed: input.stats.targetsMissed,
    wrongTaps: input.stats.wrongTaps,
    accuracy: accuracyOf(input.stats.targetsHit, input.stats.targetsMissed),
    reactions: [...input.stats.reactions],
    speedFactors: [...input.stats.speedFactors],
    meanReactionMs: meanOf(input.stats.reactions),
    bestReactionMs: bestOf(input.stats.reactions),
    meanSpeed: meanSpeedOf(input.stats.speedFactors),
    bestStreak: input.stats.bestStreak,
    perfectRounds: input.stats.perfectRounds,
    roundsPlayed: input.stats.roundsPlayed,
    roundsPassed: input.stats.roundsPassed,
    finalWindowMs: input.finalWindowMs,
    initialWindowMs: input.params.initialWindowMs,
    count: input.params.count,
    targetRadius: input.params.targetRadius,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: TapRushRawResult;
 ...
 */
export async function persistTapRushSession(
  ...
  try {
    ...
    console.error(`[speed-tap-rush] failed to persist completed session ${record.id}`, error);
