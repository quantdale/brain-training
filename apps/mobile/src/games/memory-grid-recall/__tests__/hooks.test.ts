...
describe("createGridRecallQaForceStateHooks", () => {
  it("dispatches the matching reducer action for each hook", () => {
    const dispatch = jest.fn();
    const hooks = createGridRecallQaForceStateHooks(dispatch);
  ...
  it("is bound to the game id and runs in dev builds", () => {
    const hooks = createGridRecallQaForceStateHooks(jest.fn());
...
describe("createGridRecallTutorialLifecycle", () => {
  it("shows on first play, completes, and honors replays", () => {
    const tutorial = createGridRecallTutorialLifecycle();
