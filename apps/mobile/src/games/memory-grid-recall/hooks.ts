/**
 * Grid Recall game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createGridRecallQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { GridRecallAction } from "./types";

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createGridRecallQaForceStateHooks(
  dispatch: Dispatch<GridRecallAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Grid Recall game. Uses the SDK reference lifecycle
 ...
 */
export function createGridRecallTutorialLifecycle(
