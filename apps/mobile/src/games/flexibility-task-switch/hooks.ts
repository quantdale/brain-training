/**
 * Task Switch game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createFlexibilityTaskSwitchQaForceStateHooks` implements the SDK
...
import type { FlexibilityTaskSwitchAction } from "./types";

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createFlexibilityTaskSwitchQaForceStateHooks(
  dispatch: Dispatch<FlexibilityTaskSwitchAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Task Switch game. Uses the SDK reference lifecycle
 * over the provided store (an in-memory store by default; the db layer may
 * plug a persistent `TutorialStore` in a later wave).
 */
export function createFlexibilityTaskSwitchTutorialLifecycle(
