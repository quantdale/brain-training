/**
 * Session result building + persistence for the Task Switch game.
 *
...
import { GAME_ID, switchCostMsOf } from "./types";
import type {
...
export interface BuildRawResultInput {
  ...
  /** Null only for non-procedural games; Task Switch always ships a generator. */
  readonly generatorVersion: string | null;
  readonly scoringVersion: string;
  readonly difficulty: DifficultyLevel;
  readonly params: FlexibilityTaskSwitchDifficultyParams;
  /** Effective per-trial switch rate at session end (constant per session). */
  readonly finalSwitchRate: number;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: FlexibilityTaskSwitchStats;
 ...
 */
export function buildFlexibilityTaskSwitchRawResult(
  input: BuildRawResultInput,
): FlexibilityTaskSwitchRawResult {
  const generatorInfo = {
    rounds: input.params.rounds,
    switchRate: input.params.switchRate,
    taskPool: input.params.taskPool.join(","),
    ...
    finalSwitchRate: input.finalSwitchRate,
  ...
  return {
    ...
    correctPicks: input.stats.correctPicks,
    mistakes: input.stats.mistakes,
    accuracy: accuracyOf(input.stats.correctPicks, input.stats.roundsPlayed),
    bestStreak: input.stats.bestStreak,
    totalResponseMs: input.stats.totalResponseMs,
    scoredPicks: input.stats.scoredPicks,
    speedScore: speedScoreOf(
      input.stats.totalResponseMs,
      input.stats.scoredPicks,
      input.params.speedTargetMs,
    ),
    switchPlayed: input.stats.switchPlayed,
    switchCorrect: input.stats.switchCorrect,
    switchAccuracy: switchAccuracyOf(
      input.stats.switchCorrect,
      input.stats.switchPlayed,
    ),
    repeatPlayed: input.stats.repeatPlayed,
    repeatCorrect: input.stats.repeatCorrect,
    switchCostMs: switchCostMsOf(input.stats),
    taskPool: input.params.taskPool,
    numColors: input.params.numColors,
    numShapes: input.params.numShapes,
    numNumbers: input.params.numNumbers,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: FlexibilityTaskSwitchRawResult;
 ...
 */
export async function persistFlexibilityTaskSwitchSession(
  ...
  try {
    ...
    console.error(
      `[flexibility-task-switch] failed to persist completed session ${record.id}`,
      error,
    );
