...
describe('createFlexibilityTaskSwitchQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn();
    const hooks = createFlexibilityTaskSwitchQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createFlexibilityTaskSwitchQaForceStateHooks(jest.fn());
...
describe('createFlexibilityTaskSwitchTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createFlexibilityTaskSwitchTutorialLifecycle();
