 ...
 *
 * Renders a pure state machine (`wordScrambleGameReducer`) and owns the side
...
export default function WordScrambleScreen(props: WordScrambleScreenProps = {}) {
  ...
  const [state, dispatch] = useReducer(wordScrambleGameReducer, undefined, createInitialWordScrambleState);
  ...
  // pipeline, and persist atomically.
  useEffect(() => {
    if (
      state.phase !== 'results' ||
  ...
  const pauseSession = useCallback(() => {
    ...
    if (
      !(current.phase === 'play' || current.phase === 'roundResult') ||
      current.paused
    ) {
      return;
    }
