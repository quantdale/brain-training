...
import type { WordScrambleRawResult, WordScrambleStats } from '../types';
...
function buildRaw(overrides: Partial<Parameters<typeof buildWordScrambleRawResult>[0]> = {}): WordScrambleRawResult {
  return buildWordScrambleRawResult({
    gameVersion: '1.0.0',
    generatorVersion: '1.0.0',
    scoringVersion: '1.0.0',
    difficulty: 'normal',
    params: WORD_SCRAMBLE_DIFFICULTY_PARAMS.normal,
    challengeRating: 0.5,
    seed: '42',
    stats: STATS,
    forced: false,
...
describe('buildWordScrambleRawResult', () => {
...
describe('buildSessionRecord', () => {
  it('maps the session onto the db record shape', () => {
    ...
    const record: GameSessionRecord = buildSessionRecord({
      sessionId: 'ws-x1',
      rawResult: raw,
      difficulty: { level: 'normal', challengeRating: 0.5, parameters: { ...WORD_SCRAMBLE_DIFFICULTY_PARAMS.normal } },
      normalized,
      xp: 0,
      startedAtMs: 1_000,
...
describe('persistWordScrambleSession', () => {
  ...
  it('logs and returns failure instead of throwing when the db fails', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      '[word-scramble] failed to persist completed session ws-p1',
      expect.any(Error),
    );
  });
});
