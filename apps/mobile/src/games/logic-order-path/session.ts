/**
 * Session result building + persistence for the Order Path game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: OrderPathDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: OrderPathStats;
 ...
 * full reproducibility envelope (seed, versions, difficulty, generator
 * details) per the SDK generator rule and constitution §21 — old results stay
 * interpretable across generator/scoring changes.
 */
export function buildOrderPathRawResult(input: BuildRawResultInput): OrderPathRawResult {
  const generatorInfo = {
    itemCount: input.params.itemCount,
    edgeDensityTarget: input.params.edgeDensityTarget,
  ...
  const accuracy =
    input.stats.roundsPlayed > 0 ? input.stats.roundsCorrect / input.stats.roundsPlayed : 0;
 ...
 * any other string is hashed with FNV-1a (32-bit) so the stored value is
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: OrderPathRawResult;
 ...
 * + profile touch). A failure is logged and reported to the caller; it must
 * never crash the game.
 */
export async function persistOrderPathSession(
  ...
  try {
    ...
    console.error(`[logic-order-path] failed to persist completed session ${record.id}`, error);
