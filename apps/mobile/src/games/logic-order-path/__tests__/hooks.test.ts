...
describe('createOrderPathQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn();
    const hooks = createOrderPathQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createOrderPathQaForceStateHooks(jest.fn());
...
describe('createOrderPathTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createOrderPathTutorialLifecycle();
