/**
 * Session result building + persistence for the Quick Compare game.
 *
...
import { quickCompareParamsToRecord } from './difficulty';
...
export interface BuildRawResultInput {
  ...
  readonly params: QuickCompareDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: QuickCompareStats;
...
/**
 * Build the persisted raw result. It carries the round statistics plus the
 * full reproducibility envelope (seed, versions, difficulty, generator info)
 * per the SDK generator rule and constitution §21.
 */
export function buildQuickCompareRawResult(input: BuildRawResultInput): QuickCompareRawResult {
  const generatorInfo = {
    rounds: input.params.rounds,
    windowMs: input.windowMs,
    maxValue: input.params.maxValue,
    optionCount: input.params.optionCount,
    promptTypeMask: quickCompareParamsToRecord(input.params).promptTypeMask,
  ...
  return {
    ...
    roundsTotal: input.stats.roundsTotal,
    roundsCorrect: input.stats.roundsCorrect,
    roundsWrong: input.stats.roundsWrong,
    roundsMissed: input.stats.roundsMissed,
    accuracy: accuracyOf(input.stats.roundsCorrect, input.stats.roundsTotal),
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: QuickCompareRawResult;
 ...
 */
export async function persistQuickCompareSession(
  ...
  try {
    ...
    console.error(`[speed-quick-compare] failed to persist completed session ${record.id}`, error);
