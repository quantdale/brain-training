...
import {
  createQuickCompareQaForceStateHooks,
...
describe('createQuickCompareQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    ...
    const hooks = createQuickCompareQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createQuickCompareQaForceStateHooks(jest.fn());
...
describe('createQuickCompareTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createQuickCompareTutorialLifecycle();
