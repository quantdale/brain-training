/**
 * Session result building + persistence for the Next in Sequence game.
 *
...
import type { LogicDifficultyParams, LogicRawResult, LogicStats } from './types';
...
export interface BuildRawResultInput {
  ...
  /** Recipe tier of the final round (adaptive: where the player ended up). */
  readonly finalTier: number;
  readonly seed: string;
  readonly stats: LogicStats;
 ...
 */
export function buildLogicRawResult(input: BuildRawResultInput): LogicRawResult {
  const generatorInfo = {
    rounds: input.params.rounds,
    recipeTier: input.params.recipeTier,
    visibleLength: input.params.visibleLength,
    minValue: input.params.minValue,
    maxValue: input.params.maxValue,
    referenceMs: input.params.referenceMs,
  ...
  return {
    ...
    totalMs: input.stats.totalMs,
    targetMs: input.stats.targetMs,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: LogicRawResult;
 ...
 */
export async function persistLogicSession(
  ...
  try {
    ...
    console.error(`[logic-next-sequence] failed to persist completed session ${record.id}`, error);
