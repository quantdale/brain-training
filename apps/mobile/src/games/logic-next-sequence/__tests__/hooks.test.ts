...
describe('createLogicQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn();
    const hooks = createLogicQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createLogicQaForceStateHooks(jest.fn() as (action: LogicAction) => void);
...
describe('createLogicTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createLogicTutorialLifecycle();
