...
import type { LogicRawResult, LogicStats } from '../types';
...
function buildRaw(overrides: Partial<Parameters<typeof buildLogicRawResult>[0]> = {}): LogicRawResult {
  return buildLogicRawResult({
    gameVersion: '1.0.0',
...
describe('buildLogicRawResult', () => {
  ...
  it('reflects the stats and timing', () => {
    const raw = buildRaw();
    expect(raw.score).toBe(450);
    expect(raw.roundsPlayed).toBe(4);
    expect(raw.roundsPassed).toBe(3);
    expect(raw.bestStreak).toBe(2);
    expect(raw.totalMs).toBe(36_000);
    expect(raw.targetMs).toBe(32_000);
    expect(raw.fastestMs).toBe(7_500);
  });
});
  ...
  it('maps the session onto the db record shape', () => {
    ...
    const record: GameSessionRecord = buildSessionRecord({
      ...
      difficulty: {
        level: 'normal',
        challengeRating: 0.5,
        parameters: { ...LOGIC_DIFFICULTY_PARAMS.normal },
      },
      normalized,
      xp: 0,
      startedAtMs: 1_000,
...
describe('persistLogicSession', () => {
  ...
  it('logs and returns failure instead of throwing when the db fails', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      '[logic-next-sequence] failed to persist completed session logic-p1',
      expect.any(Error),
    );
  });
});
