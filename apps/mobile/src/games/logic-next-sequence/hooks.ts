/**
 * Next in Sequence — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createLogicQaForceStateHooks` implements the SDK `QaForceStateHooks`
 ...
 *
 * Note: the SDK contract has no `forceTimeout` slot and this game has no
 * per-round countdown timer (rounds end when the player answers), so the
 * force surface is force-win / force-lose / force-state (seed + difficulty).
 */
...
export function createLogicQaForceStateHooks(
 ...
 * provided store (an in-memory store by default; the db layer may plug a
 * persistent `TutorialStore` in a later wave — see GAME_SDK.md).
 */
export function createLogicTutorialLifecycle(
