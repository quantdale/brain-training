/**
 * QaPanel — dev-only force-state controls for the Reaction Time game.
 *
...
export interface QaPanelProps {
  onForceWin: () => void;
  onForceLose: () => void;
  /** Game-specific: fails the current round as a timeout (dev-only). */
  onForceTimeout: () => void;
}
      ...
      {open ? (
        <View style={styles.panel} testID={testId(GAME_ID, 'qa-panel')}>
          <ThemedText type="caption" themeColor="textSecondary">
            Force-state hooks (assertDevOnly) — win/lose end the session instantly;
            timeout fails the current round.
          </ThemedText>
