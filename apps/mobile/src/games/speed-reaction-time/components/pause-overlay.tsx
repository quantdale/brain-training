/**
 * PauseOverlay — pause surface for the Reaction Time game.
 *
 ...
 * paused the trigger is hidden from the accessibility tree by the screen
 * (importantForAccessibility / accessibilityElementsHidden).
 *
 * Pause semantics for timing: while paused, both the GO timer and the reaction
 * window are frozen (their timers are cancelled). Resuming restarts the wait
 * delay from scratch and — if the pause happened after GO — re-displays the GO
 * signal with a fresh monotonic `goAtMs` (see screen.tsx), so a pause can
 * never manufacture a reaction time.
 *
 ...
 * requirement is met with a solid themed surface instead (see packet report).
 */
