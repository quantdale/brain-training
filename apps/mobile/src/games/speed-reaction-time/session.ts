/**
 * Session result building + persistence for the Reaction Time game.
 *
...
import { medianOf, meanOf, bestOf } from './scoring';
import { GAME_ID } from './types';
import type { SpeedDifficultyParams, SpeedRawResult, SpeedStats } from './types';
...
export interface BuildRawResultInput {
  ...
  readonly params: SpeedDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: SpeedStats;
...
/**
 * Build the persisted raw result. It carries the reaction statistics plus the
 * full reproducibility envelope (seed, versions, difficulty, generator info)
 * per the SDK generator rule and constitution §21.
 */
export function buildSpeedRawResult(input: BuildRawResultInput): SpeedRawResult {
  const generatorInfo = {
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: SpeedRawResult;
 ...
 */
export async function persistSpeedSession(
  ...
  try {
    ...
    console.error(
      `[speed-reaction-time] failed to persist completed session ${record.id}`,
      error,
    );
