/**
 * Session result building + persistence for the Pair Recall game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: PairRecallDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: PairRecallStats;
 ...
 */
export function buildPairRecallRawResult(
  input: BuildRawResultInput,
): PairRecallRawResult {
  const generatorInfo = {
    initialPairCount: input.params.initialPairCount,
    maxPairCount: input.params.maxPairCount,
    studyMs: input.params.studyMs,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: PairRecallRawResult;
 ...
 */
export async function persistPairRecallSession(
  ...
  try {
    ...
    console.error(
      `[memory-pair-recall] failed to persist completed session ${record.id}`,
      error,
    );
