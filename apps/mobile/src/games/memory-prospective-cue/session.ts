/**
 * Session result building + persistence for the Cue Keeper game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: ProspectiveCueDifficultyParams;
  readonly challengeRating: number;
  /** Per-item window reached by the end of the session (ms). */
  readonly finalItemMs: number;
  readonly seed: string;
  readonly stats: ProspectiveCueStats;
 ...
 */
export function buildProspectiveCueRawResult(
  input: BuildRawResultInput,
): ProspectiveCueRawResult {
  const generatorInfo = {
    initialSignalCount: input.params.initialSignalCount,
    maxSignalCount: input.params.maxSignalCount,
    initialItemMs: input.params.initialItemMs,
    minItemMs: input.params.minItemMs,
    streamLen: input.params.streamLen,
  ...
  return {
    ...
    accuracy: itemAccuracyOf(
      input.stats.correctResponses,
      input.stats.totalItems,
    ),
    signalAccuracy: signalAccuracyOf(
      input.stats.signalHits,
      input.stats.totalSignals,
    ),
    bestStreak: input.stats.bestStreak,
    initialSignalCount: input.params.initialSignalCount,
    maxSignalCount: input.params.maxSignalCount,
    itemMs: input.finalItemMs,
    streamLen: input.params.streamLen,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: ProspectiveCueRawResult;
 ...
 */
export async function persistProspectiveCueSession(
  ...
  try {
    ...
    console.error(
      `[memory-prospective-cue] failed to persist completed session ${record.id}`,
      error,
    );
