...
describe("createRunningOrderQaForceStateHooks", () => {
  it("dispatches the matching reducer action for each hook", () => {
    const dispatch = jest.fn();
    const hooks = createRunningOrderQaForceStateHooks(dispatch);
  ...
  it("is bound to the game id and runs in dev builds", () => {
    const hooks = createRunningOrderQaForceStateHooks(jest.fn());
...
describe("createRunningOrderTutorialLifecycle", () => {
  it("shows on first play, completes, and honors replays", () => {
    const tutorial = createRunningOrderTutorialLifecycle();
