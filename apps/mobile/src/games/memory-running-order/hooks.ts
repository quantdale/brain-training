/**
 * Running Order game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createRunningOrderQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { RunningOrderAction } from "./types";

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createRunningOrderQaForceStateHooks(
  dispatch: Dispatch<RunningOrderAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Running Order game. Uses the SDK reference
 * lifecycle over the provided store (an in-memory store by default; the db
 * layer may plug a persistent `TutorialStore` in a later wave — see GAME_SDK.md).
 */
export function createRunningOrderTutorialLifecycle(
