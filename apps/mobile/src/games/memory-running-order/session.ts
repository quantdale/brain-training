/**
 * Session result building + persistence for the Running Order game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: RunningOrderDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: RunningOrderStats;
 ...
 */
export function buildRunningOrderRawResult(
  input: BuildRawResultInput,
): RunningOrderRawResult {
  const generatorInfo = {
    streamLen: input.params.streamLen,
    flashMs: input.params.flashMs,
    rounds: input.params.rounds,
    initialRecallLength: input.params.initialRecallLength,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: RunningOrderRawResult;
 ...
 */
export async function persistRunningOrderSession(
  ...
  try {
    ...
    console.error(
      `[memory-running-order] failed to persist completed session ${record.id}`,
      error,
    );
