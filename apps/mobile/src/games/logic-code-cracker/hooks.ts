/**
 * Code Cracker game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createCodeCrackerQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { CodeCrackerAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createCodeCrackerQaForceStateHooks(
  dispatch: Dispatch<CodeCrackerAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Code Cracker game. Uses the SDK reference lifecycle
 ...
 */
export function createCodeCrackerTutorialLifecycle(
