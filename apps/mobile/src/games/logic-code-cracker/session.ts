/**
 * Session result building + persistence for the Code Cracker game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: CodeCrackerDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: CodeCrackerStats;
 ...
 */
export function buildCodeCrackerRawResult(input: BuildRawResultInput): CodeCrackerRawResult {
  const generatorInfo = {
  ...
  return {
    ...
    roundsSolved: input.stats.roundsSolved,
    accuracy: accuracyOf(input.stats.roundsSolved, input.stats.roundsPlayed),
    ...
    forced: input.forced,
    guessHistory: input.guessHistory,
    generatorInfo,
    diagnosticMetadata,
  };
}
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: CodeCrackerRawResult;
 ...
 */
export async function persistCodeCrackerSession(
  ...
  try {
    ...
    console.error(`[logic-code-cracker] failed to persist completed session ${record.id}`, error);
