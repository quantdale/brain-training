/**
 * Session result building + persistence for the Math Missing Operator game.
 *
...
import { accuracyOf, avgResponseMs } from './scoring';
...
export interface BuildRawResultInput {
  ...
  readonly stats: MathMissingOperatorStats;
 ...
 */
export function buildMathMissingOperatorRawResult(
  input: BuildRawResultInput,
): MathMissingOperatorRawResult {
  const generatorInfo = {
    minA: input.params.minA,
    maxA: input.params.maxA,
    minB: input.params.minB,
    maxB: input.params.maxB,
    operators: input.params.operators.join(','),
    baseTimeMs: input.params.baseTimeMs,
    minTimeMs: input.params.minTimeMs,
    shrinkPerRound: input.params.shrinkPerRound,
  ...
  return {
    ...
    avgResponseMs: avgResponseMs(input.stats),
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: MathMissingOperatorRawResult;
 ...
 */
export async function persistMathMissingOperatorSession(
  ...
  try {
    ...
    console.error(
      `[math-missing-operator] failed to persist completed session ${record.id}`,
      error,
    );
