/**
 * Session result building + persistence for the Cue Shift game.
 *
...
import type { FlexibilityCueDifficultyParams, FlexibilityCueRawResult, FlexibilityCueStats } from './types';
 ...
 */
export function buildFlexibilityCueRawResult(input: BuildRawResultInput): FlexibilityCueRawResult {
  const generatorInfo = {
  ...
  return {
    ...
    switchAccuracy: switchAccuracyOf(input.stats.switchCorrect, input.stats.switchPlayed),
    numShapes: input.params.numShapes,
    numColors: input.params.numColors,
    numNumbers: input.params.numNumbers,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: FlexibilityCueRawResult;
 ...
 */
export async function persistFlexibilityCueSession(
  ...
  try {
    ...
    console.error(`[flexibility-cue-shift] failed to persist completed session ${record.id}`, error);
