/**
 * Cue Shift game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createFlexibilityCueQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { FlexibilityCueAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createFlexibilityCueQaForceStateHooks(
  dispatch: Dispatch<FlexibilityCueAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Cue Shift game. Uses the SDK reference lifecycle
 * over the provided store (an in-memory store by default; the db layer may
 * plug a persistent `TutorialStore` in a later wave).
 */
export function createFlexibilityCueTutorialLifecycle(
