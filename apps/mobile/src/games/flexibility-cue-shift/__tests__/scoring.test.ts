...
import type { FlexibilityCueRawResult } from '../types';

function rawResult(overrides: Partial<FlexibilityCueRawResult>): FlexibilityCueRawResult {
  return {
    score: 0,
    totalRounds: 10,
    roundsPlayed: 0,
...
describe('perfectSessionScore', () => {
  it('is 150 per round (perfect accuracy at instant speed)', () => {
    expect(perfectSessionScore(FLEXIBILITY_CUE_DIFFICULTY_PARAMS.easy)).toBe(8 * 150);
    expect(perfectSessionScore(FLEXIBILITY_CUE_DIFFICULTY_PARAMS.normal)).toBe(10 * 150);
    expect(perfectSessionScore(FLEXIBILITY_CUE_DIFFICULTY_PARAMS.expert)).toBe(12 * 150);
  });
});
...
describe('switchAccuracyOf', () => {
  it('computes the rule-switch ratio and guards zero played', () => {
...
describe('normalizeFlexibilityCueResult (documented formula)', () => {
  it('scores a perfect fast run at 1.0', () => {
    const normalized = normalizeFlexibilityCueResult(
      rawResult({
        roundsPlayed: 10,
        correctPicks: 10,
        totalResponseMs: 0,
  ...
  it('is 0 when no round was correct', () => {
    const normalized = normalizeFlexibilityCueResult(
      rawResult({ roundsPlayed: 10, correctPicks: 0, switchPlayed: 3, switchCorrect: 0 }),
      { gameId: 'flexibility-cue-shift', difficulty: 'normal', durationMs: 0 },
    );
  ...
  it('blends speed and switch accuracy at 20% each on top of accuracy', () => {
    ...
    const slow = normalizeFlexibilityCueResult(
      rawResult({
        roundsPlayed: 10,
        correctPicks: 10,
    ...
    const accurate = normalizeFlexibilityCueResult(
      rawResult({
        roundsPlayed: 10,
        correctPicks: 8,
        totalResponseMs: 0,
  ...
  it('keeps the raw snapshot for diagnostics', () => {
    const raw = rawResult({ roundsPlayed: 1, correctPicks: 1, switchPlayed: 1, switchCorrect: 1 });
