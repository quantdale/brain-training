...
type DispatchFn = (action: FlexibilityCueAction) => void;

describe('createFlexibilityCueQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn<DispatchFn>();
    const hooks = createFlexibilityCueQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createFlexibilityCueQaForceStateHooks(jest.fn<DispatchFn>());
...
describe('createFlexibilityCueTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createFlexibilityCueTutorialLifecycle();
