/**
 * Session result building + persistence for the Deduction Table game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly stats: LogicDeductionStats;
...
/**
 * Build the persisted raw result. It carries the score statistics plus the full
 * reproducibility envelope (seed, versions, difficulty, per-round outcomes) per
 * the SDK generator rule and constitution §21 — old results stay interpretable
 * across generator/content updates.
 */
export function buildLogicDeductionRawResult(
  input: BuildRawResultInput,
): LogicDeductionRawResult {
  const generatorInfo = {
    entityCount: input.params.entityCount,
    attributeCount: input.params.attributeCount,
    clueCount: input.params.clueCount,
 ...
 * any other string is hashed with FNV-1a (32-bit) so the stored value is stable.
 */
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: LogicDeductionRawResult;
...
/**
 * Persist a completed session through `completeSession` (atomic). A failure is
 * logged and reported to the caller; it must never crash the game.
 */
export async function persistLogicDeductionSession(
  ...
  try {
    ...
    console.error(
      `[logic-deduction-table] failed to persist completed session ${record.id}`,
      error,
    );
