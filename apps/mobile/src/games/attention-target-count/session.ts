/**
 * Session result building + persistence for the Target Count game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly stats: TargetCountStats;
 ...
 */
export function buildTargetCountRawResult(input: BuildRawResultInput): TargetCountRawResult {
  const generatorInfo = {
    rows: input.params.rows,
    cols: input.params.cols,
    distractorClasses: input.params.distractorClasses,
    targetCountLo: input.params.targetCountRange[0],
    targetCountHi: input.params.targetCountRange[1],
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: TargetCountRawResult;
 ...
 */
export async function persistTargetCountSession(
  ...
  try {
    ...
    console.error(`[attention-target-count] failed to persist completed session ${record.id}`, error);
