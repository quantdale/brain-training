/**
 * Session result building + persistence for the Rule Grid game.
 *
...
import type { RuleGridDifficultyParams, RuleGridRawResult, RuleGridStats } from './types';
 ...
 */
export function buildRuleGridRawResult(input: BuildRawResultInput): RuleGridRawResult {
  const generatorInfo = {
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: RuleGridRawResult;
 ...
 */
export async function persistRuleGridSession(
  ...
  try {
    ...
    console.error(`[logic-rule-grid] failed to persist completed session ${record.id}`, error);
