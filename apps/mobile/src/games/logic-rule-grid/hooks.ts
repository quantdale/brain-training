/**
 * Rule Grid game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createRuleGridQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { RuleGridAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createRuleGridQaForceStateHooks(
  dispatch: Dispatch<RuleGridAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Rule Grid game. Uses the SDK reference lifecycle
 ...
 */
export function createRuleGridTutorialLifecycle(
