/**
 * Session result building + persistence for the Card Sort game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: FlexibilityDifficultyParams;
  /** Effective switch frequency at session end (constant for fixed levels). */
  readonly finalSwitchEvery: number;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: FlexibilityStats;
 ...
 */
export function buildFlexibilityRawResult(input: BuildRawResultInput): FlexibilityRawResult {
  const generatorInfo = {
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: FlexibilityRawResult;
 ...
 */
export async function persistFlexibilitySession(
  ...
  try {
    ...
    console.error(`[flexibility-card-sort] failed to persist completed session ${record.id}`, error);
