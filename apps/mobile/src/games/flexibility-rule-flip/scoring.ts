 ...
 *
 * Raw scoring (speed + accuracy + a switch-correct bonus):
 *
 ...
 * trial scores 170; the fastest possible repeat trial scores 150.
 *
 ...
 *   value           = clamp01( accuracy * (0.55 + 0.25 * speedScore + 0.20 * switchAccuracy) )
 *
 * Accuracy is the base; speed contributes up to 25% and switch-rule accuracy
 * (how well the player re-anchors right after a rule flip — the flexibility
 * diagnostic) contributes up to 20% of the blend. A perfect, instant,
 * switch-perfect run reaches 1.0. Difficulty itself is deliberately NOT folded
...
/**
 * Raw score of a single trial (correctness + speed + optional switch bonus).
...
export function trialRawScore(
  ...
  return roundScore(true, responseMs, speedTargetMs) + (isSwitch ? SWITCH_CORRECT_BONUS : 0);
}
...
/** Score of a hypothetically perfect plan (switch trials also earn the bonus). */
export function perfectPlanScore(plan: readonly { isSwitch: boolean }[]): number {
  return plan.reduce((sum, round) => sum + trialRawScore(true, 0, 1, round.isSwitch), 0);
}
...
export function normalizeFlexibilityRuleFlipResult(
  ...
  const value = clamp01(accuracy * (0.55 + 0.25 * speed + 0.2 * switchAccuracy));
