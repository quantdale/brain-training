/**
 * Session result building + persistence for the Rule Flip game.
 *
 ...
 */
export function buildFlexibilityRuleFlipRawResult(input: BuildRawResultInput): FlexibilityRuleFlipRawResult {
  const generatorInfo = {
  ...
  return {
    ...
    repeatAccuracy: repeatAccuracyOf(input.stats),
    numShapes: input.params.numShapes,
    numColors: input.params.numColors,
    numNumbers: input.params.numNumbers,
    flipRate: input.params.flipRate,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: FlexibilityRuleFlipRawResult;
 ...
 */
export async function persistFlexibilityRuleFlipSession(
  ...
  try {
    ...
    console.error(`[flexibility-rule-flip] failed to persist completed session ${record.id}`, error);
