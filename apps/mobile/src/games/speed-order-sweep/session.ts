/**
 * Session result building + persistence for the Order Sweep game.
 *
...
export interface BuildRawResultInput {
  ...
  readonly params: OrderSweepDifficultyParams;
  readonly challengeRating: number;
  readonly seed: string;
  readonly stats: OrderSweepStats;
  /** Time budget of the final round (drives the adaptive rating). */
...
/**
 * Build the persisted raw result. It carries the sweep statistics plus the
 * full reproducibility envelope (seed, versions, difficulty, generator info)
 * per the SDK generator rule and constitution §21.
 */
export function buildOrderSweepRawResult(input: BuildRawResultInput): OrderSweepRawResult {
  const totalTokens = input.params.count * input.params.rounds;
  const generatorInfo = {
    rounds: input.params.rounds,
    count: input.params.count,
    columns: input.params.columns,
  ...
  return {
    ...
    totalTokens,
    tokensCleared: input.stats.tokensCleared,
    tokensExpired: input.stats.tokensExpired,
    wrongTaps: input.stats.wrongTaps,
    clearRatio: clearRatioOf(input.stats.tokensCleared, totalTokens),
    gaps: [...input.stats.gaps],
    meanGapMs: meanOf(input.stats.gaps),
    bestGapMs: bestOf(input.stats.gaps),
    ...
    roundsCleared: input.stats.roundsCleared,
...
export interface BuildSessionRecordInput {
  ...
  readonly rawResult: OrderSweepRawResult;
 ...
 * + profile touch; no ledger entry is requested here — the rating service owns
 * the gameplay currency award when configured). A failure is logged and
 * reported to the caller; it must never crash the game.
 */
export async function persistOrderSweepSession(
  ...
  try {
    ...
    console.error(`[speed-order-sweep] failed to persist completed session ${record.id}`, error);
