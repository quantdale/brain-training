/**
 * Order Sweep game — dev-only QA hooks and tutorial lifecycle wiring.
 *
 * `createOrderSweepQaForceStateHooks` implements the SDK `QaForceStateHooks`
...
import type { OrderSweepAction } from './types';

/** Create the QA hooks bound to a reducer dispatch (dev-only methods). */
export function createOrderSweepQaForceStateHooks(
  dispatch: Dispatch<OrderSweepAction>,
): QaForceStateHooks {
  return {
    gameId: GAME_ID,
    forceWin: () => {
...
/**
 * Tutorial lifecycle for the Order Sweep game. Uses the SDK reference lifecycle
 ...
 */
export function createOrderSweepTutorialLifecycle(
