...
import type { NumberLineAction } from '../types';

describe('createNumberLineQaForceStateHooks', () => {
  it('dispatches the matching reducer action for each hook', () => {
    const dispatch = jest.fn<(action: NumberLineAction) => void>();
    const hooks = createNumberLineQaForceStateHooks(dispatch);
  ...
  it('is bound to the game id and runs in dev builds', () => {
    const hooks = createNumberLineQaForceStateHooks(jest.fn());
...
describe('createNumberLineTutorialLifecycle', () => {
  it('shows on first play, completes, and honors replays', () => {
    const tutorial = createNumberLineTutorialLifecycle();
