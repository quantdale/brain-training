...
import { exportLocalData, serializeBackup, parseAndValidateBackup, previewImport } from '../index';
...
describe('previewImport', () => {
  it('produces a valid dry-run report for merge without mutating', async () => {
    ...
    expect(preview.notes.some((n) => n.toLowerCase().includes('merge'))).toBe(true);
  });
  ...
  it('reports a destructive warning for replace and counts all entities', async () => {
    ...
    expect(preview.valid).toBe(true);
    expect(preview.counters.sessionsAdded).toBe(2);
    expect(preview.notes.some((n) => /destructive|overwrite/i.test(n))).toBe(true);
  });
