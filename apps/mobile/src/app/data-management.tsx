...
export default function DataManagementScreen() {
  ...
  const onExport = useCallback(async () => {
    ...
    try {
      ...
      setExportText(text);
      setMessage(`Exported ${env.data.gameSessions.length} sessions, ${env.data.currencyLedger.length} ledger entries.`);
    } catch (e) {
  ...
  const onPreview = useCallback(
    async (mode: 'merge' | 'replace') => {
      ...
      try {
        ...
        setPreview(result);
        if (!result.valid) {
          setMessage(`Preview: rejected (${result.error?.kind}): ${result.error?.message}`);
        } else {
          setMessage(
  ...
  const onImport = useCallback(
    async (mode: 'merge' | 'replace') => {
      ...
      if (!previewResult.valid) {
        setMessage(`Import rejected: ${previewResult.error?.kind} — ${previewResult.error?.message}`);
        return;
      }
  ...
  return (
    <ScreenShell>
      ...
      <ThemedText type="caption" themeColor="textSecondary">
        Export, preview, and restore your local training data. All operations validate
        before mutating and are fully offline.
      </ThemedText>

      <ThemedView type="surface" style={styles.card} testID="data-counts">
        <ThemedText type="subtitle">Local Data</ThemedText>
        <View style={styles.countGrid}>
          <Count label="Sessions" value={counts.gameSessions} testID="data-count-sessions" />
          <Count label="Ratings" value={counts.domainRatings} testID="data-count-ratings" />
          <Count label="History" value={counts.ratingHistory} testID="data-count-history" />
          <Count label="Ledger" value={counts.currencyLedger} testID="data-count-ledger" />
          <Count label="Favorites" value={counts.gameFavorites} testID="data-count-favorites" />
          <Count label="Quests" value={counts.questProgress} testID="data-count-quests" />
        </View>
        <ThemedText type="caption" themeColor="textSecondary">
          {counts.hasProfile ? 'Profile present' : 'No profile'} · {counts.workoutInstances} workout
          instances · {counts.tutorialState} tutorial states
        </ThemedText>
      </ThemedView>
      ...
      <ThemedView type="surface" style={styles.card} testID="data-export-card">
        ...
        <ThemedText type="caption" themeColor="textSecondary">
          Creates a versioned, checksummed JSON envelope containing all canonical local
          data (sessions, ratings, ledger, quests, streak inventory, settings, etc.).
        </ThemedText>
        <Pressable
        ...
        {exportText ? (
          <View style={styles.exportBox} testID="data-export-output">
            ...
            <ThemedText type="caption" themeColor="textSecondary">
              Full backup is {exportText.length} characters. Copy the full text from the
              export (long-press to select). For sharing, paste into a file or cloud note.
            </ThemedText>
          </View>
        ) : null}
      </ThemedView>
      ...
      <ThemedView type="surface" style={styles.card} testID="data-import-card">
        ...
        <ThemedText type="caption" themeColor="textSecondary">
          Paste a previously exported backup JSON. Preview first to see what would change
          (no data is written during preview). Then choose Merge (add new, keep existing)
          or Replace (overwrite everything).
        </ThemedText>
        <TextInput
        ...
        {preview ? (
          <View style={styles.previewBox} testID="data-preview-output">
            <ThemedText type="smallBold">
              Preview: {preview.valid ? 'Valid' : `Invalid (${preview.error?.kind})`}
            </ThemedText>
            <ThemedText type="caption" themeColor="textSecondary">
              {preview.valid
      ...
      <ThemedView type="surface" style={styles.card} testID="data-wipe-card">
        ...
        <ThemedText type="caption" themeColor="textSecondary">
          Permanently deletes all sessions, ratings, ledger, quests, and settings.
          Offer a backup first — export above before wiping. Type DELETE to confirm.
        </ThemedText>
        <TextInput
        ...
        <Pressable
          ...
          onPress={onWipe}>
          <ThemedView type={wipeConfirm === 'DELETE' ? 'danger' : 'surface'} style={styles.pill}>
            <ThemedText type="smallBold" themeColor={wipeConfirm === 'DELETE' ? 'danger' : 'textSecondary'}>
              Wipe Local Data
            </ThemedText>
          </ThemedView>
        </Pressable>
