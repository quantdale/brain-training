...
export class WorkoutRepository {
  ...
  async advance(date: string): Promise<WorkoutInstance> {
    ...
    const nextIndex = Math.min(current.currentIndex + 1, current.gameIds.length);
    const status: WorkoutStatus = nextIndex >= current.gameIds.length ? 'completed' : 'active';
   ...
   * The currency cost is handled by the caller (economy layer, task 7.4/6.5).
   */
  async applyReroll(
    date: string,
    newGameIds: string[],
