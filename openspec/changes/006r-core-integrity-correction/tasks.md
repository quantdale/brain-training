...
- [x] 10.3 Migrate incrementally without centralizing game mechanics; prove representative canary games still work. — ALL 20 games now migrated to the shared primitives: per-game `button.tsx` is a re-export adapter of `GameButton`; `pause-overlay.tsx`/`qa-panel.tsx` are thin `PauseOverlay`/`QaPanelShell` adapters (injecting `GAME_ID`); `tutorial.tsx` wraps content in `TutorialFrame`; and `screen.tsx` uses shared `DifficultySelector`/`SessionHeader`/`StatRow` (removing the local `StatRow` copies). Canary batch: memory, memory-sequence-memory, speed-reaction-time, speed-color-match, math-fast-math, spatial-mental-rotation; language batch: language-word-match, language-sentence-builder, language-word-scramble; final batch: logic-code-cracker, logic-next-sequence, math-equation-builder, math-missing-operator, memory-pattern-tap-back, spatial-transform-match, speed-tap-rush. All 20 game mechanics stay local; shared shell's `extraActions` keeps per-game QA (force-timeout/force-perfect) local. Full Jest 190 suites / 2272 tests green, tsc clean, lint clean across all games, `QaPanelShell` gap closed.
