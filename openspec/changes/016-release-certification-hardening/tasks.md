...
- [x] 3.1 Capture current emulator/SDK/WHPX/graphics/AVD versions and segfault logs — bounded inventory captured: Linux host, Android emulator 37.1.11.0, platform-tools/ADB 37.0.1, SDK platforms 35/36, Java 17 available, no Gradle command, one connected foreign `study-maker-api35` emulator, and no designated `braintraining-qa36` AVD; prior Windows/WHPX failure logs remain in the 014/015/016 checkpoints.
...
- [x] 3.10 If 3.3–3.6 all fail for infrastructure reasons, record BLOCKED with evidence; never mark PASS — Android runtime certification remains BLOCKED/NOT VALIDATED because no designated project device or physical fallback is available and the prior 37.1.11/WHPX recovery remains unstable; the foreign AVD was not adopted.
...
- [x] 8.1 Clean checkout install and all repository validators PASS — **PASS** at source head `f0d301bc1b80ed657c75af81c476ee87dbeea540`: repo-state, registry, provenance, ownership, offline, QA self-test, typecheck, lint, full Jest, web export, and Expo Doctor all pass. The local host requires the documented `npm ci --ignore-scripts` fallback because `make` is unavailable for `better-sqlite3`; clean CI completes the normal install.
- [x] 8.2 TypeScript PASS; lint 0 errors / 0 warnings — source head `f0d301bc1b80ed657c75af81c476ee87dbeea540` typecheck and `expo lint` pass; targeted changed-surface ESLint also passes.
- [x] 8.3 Jest PASS with only allowlisted skips and zero unexpected console noise — **PASS**: full Jest 489 suites / 6056 tests / 0 failures; 4 suites skipped (all opt-in performance probes matching allowlist); 5 tests skipped (all opt-in); Jest signal validation PASS with 0 unclassified skips and 0 unexpected warnings.
- [x] 8.4 Web export + Expo Doctor PASS — web export produced 20 static routes; Expo Doctor passed 21/21.
- [x] 8.5 Android clean prebuild/native build PASS — exact source SHA `f0d301bc1b80ed657c75af81c476ee87dbeea540`, Android Build Smoke `33293614561`, job `Android clean native build`, passed clean native generation, release APK compilation, release-boundary verification, and artifact upload.
...
- [x] 8.11 Confirm App CI + Repository Integrity and native smokes green on exact source SHA — App CI `33293614545`, Repository Integrity `33293614543`, Android Build Smoke `33293614561`, and iOS Build Smoke `33293614540` all PASS on exact source SHA `f0d301bc1b80ed657c75af81c476ee87dbeea540`. The older Android timeout `33239131146` on `31a6143` remains historical only; final pushed-SHA workflow IDs are recorded in the terminal checkpoint.
- [x] 8.12 Write terminal checkpoint and synchronize STATE/CURRENT_CAMPAIGN/KNOWN_ISSUES/VALIDATION/OpenSpec lifecycle — terminal checkpoint `.agent/checkpoints/016-release-certification-hardening-VALIDATED-20260830.md` records the final exact-SHA CI wave, external/manual classifications, and `VALIDATED` no-successor state.
