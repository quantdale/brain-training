...
- **Campaign 015 closure is awaiting corrective-SHA workflows:** closure candidate `9ef2531` had Repository Integrity `33225677246` PASS, while App CI `33225677247` failed only at Expo Doctor (`20/21`) because three Expo SDK patch dependencies were stale. The repair updates `expo ~57.0.18`, `expo-constants ~57.0.16`, and `expo-font ~57.0.2`; local `npm ci` and Expo Doctor `21/21` pass. Local Linux full-suite attempts crash Node workers/processes with SIGSEGV under host resource contention (isolated `language-word-scramble` passes 3/3), so local execution is `NOT VALIDATED`, not hidden with a threshold change. Dedicated `braintraining-qa36` is externally BLOCKED/NOT VALIDATED: this host has no Android SDK/ADB/emulator, and prior bounded Windows 37.1.x/WHPX attempts failed before ADB registration/segfaulted. No Critical/High product regression is currently known.
