...
- **Campaign 015 closure evidence is awaiting the closure SHA workflows:** exact-head App CI `33121632955` and Repository Integrity `33121632951` passed on `ea144d7` with 488 passed / 4 skipped suites and 6,043 passed / 5 skipped tests. Local Linux full-suite attempts crash Node workers/processes with SIGSEGV under host resource contention (isolated `language-word-scramble` passes 3/3), so local execution is `NOT VALIDATED`, not hidden with a threshold change. Dedicated `braintraining-qa36` is externally BLOCKED/NOT VALIDATED: this host has no Android SDK/ADB/emulator, and prior bounded Windows 37.1.x/WHPX attempts failed before ADB registration/segfaulted. No Critical/High product regression is currently known.
