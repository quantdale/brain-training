...
- Terminal blocked checkpoint `.agent/checkpoints/016-release-certification-hardening-BLOCKED.md` is committed at `fdbf145` (based on source head `87f43c2`). Final push/CI confirmation remains blocked by the GitHub token's missing `workflow` scope, and persistence/recovery evidence remains NOT VALIDATED after the single bounded DB probe SIGSEGV.
## Ordered workstreams
